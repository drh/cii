...
#define ones(n) ((2<<(((n)-1)%8))-1)
#define iszero(x) (XP_length(nbytes,x) == 1 && (x)[0] == 0)
#define BASE (1<<8)
#define bitop(op) int i; assert(z); assert(x); assert(y); \
	for (i = 0; i < nbytes; i++) z[i] = x[i] op y[i]; \
	return z
#define bitopi(op) assert(z); assert(x); \
	applyu(op, z, x, y); \
	return z
#define shft(fill,op) assert(x); assert(z); assert(s >= 0); \
...
static int apply(T op(T, T, T), T z, T x, long v) {
	{ T z = tmp[2]; if (v == LONG_MIN) {
			...
			z[nbytes-1] &= msb; }
	op(z, x, tmp[2]);
	return (v < -(1<<(nbits-1)) || v > (1<<(nbits-1))-1);
}
int MP_set(int n) {
	int prev = nbits;
	assert(n > 1);
	...
	if (nbytes <= 16)
		...
		tmp[0] = ALLOC(nbytes + nbytes + nbytes + 2*nbytes + 2); tmp[1] = tmp[0] + 1*nbytes;
...
T MP_cvt(int m, T z, T x) {
	...
	fill = sign(x) ? 0xFF : 0;
	if (m < nbits) {
		unsigned carry = (x[mbytes-1]^fill)&~ones(m);
...
T MP_cvtu(int m, T z, T x) {
	...
	assert(x); assert(z);
	if (m < nbits) {
		unsigned carry = x[mbytes-1]&~ones(m);
		for (i = mbytes; i < nbytes; i++)
			carry |= x[i];
		memcpy(z, x, mbytes);
		z[mbytes-1] &= ones(m);
...
T MP_mul2u(T z, T x, T y) {
	...
	XP_mul(nbytes, tmp[3], x, nbytes, y);
...
T MP_divu(T z, T x, T y) {
	...
	if (XP_div(nbytes, z, tmp[2], x, nbytes, y, tmp[3]) == 0)
...
T MP_modu(T z, T x, T y) {
	...
	if (XP_div(nbytes, tmp[2], z, x, nbytes, y, tmp[3]) == 0)
		RAISE(MP_Dividebyzero);
	return z;
}
...
T MP_addui(T z, T x, unsigned long y) {
	assert(x); assert(z);
	if (y < BASE) {
		unsigned carry = XP_sum(nbytes, z, x, y);
		carry |= z[nbytes-1]&~msb;
		z[nbytes-1] &= msb;
		if (carry)
			RAISE(MP_Overflow);
...
T MP_subui(T z, T x, unsigned long y) {
	assert(x); assert(z);
	if (y < BASE) {
		unsigned borrow = XP_diff(nbytes, z, x, y);
		borrow |= z[nbytes-1]&~msb;
		z[nbytes-1] &= msb;
		if (borrow)
			RAISE(MP_Overflow);
	} else if (applyu(MP_sub, z, x, y))
		RAISE(MP_Overflow);
	return z;
}
T MP_mului(T z, T x, unsigned long y) {
	assert(x); assert(z);
	if (y < BASE) {
		unsigned carry = XP_product(nbytes, z, x, y);
		carry |= z[nbytes-1]&~msb;
		z[nbytes-1] &= msb;
		if (carry)
			RAISE(MP_Overflow);
...
unsigned long MP_modui(T x, unsigned long y) {
	...
	if (y == 0)
		RAISE(MP_Dividebyzero);
	if (y < BASE)
		return XP_quotient(nbytes, tmp[2], x, y);
...
T MP_ashift(T z, T x, int s) { shft(sign(x)?0xFF:0,XP_rshift); }
...
void MP_fmtu(int code, va_list *app,
	...
	T x = va_arg(*app, T);
	int base = va_arg(*app, int); 	char *buf;
	assert(x);
	buf = MP_tostr(NULL, 0, base, x);
	Fmt_putd(buf, strlen(buf), put, cl,
...
void MP_fmt(int code, va_list *app,
	...
	unsigned char flags[], int width, int precision) {
	T x = va_arg(*app, T);
	int base = va_arg(*app, int), size, sx;
	char *buf;
	assert(x);
	sx = sign(x);
	if (sx) {
