	...
	for (;;) {
		...
		if (primes[j] == 0) {
			if (x > p->last)
				break;
			Fmt_print(" %d", x);
			assert(i < (int)(sizeof (primes)/sizeof (primes[0]))-2);
			primes[i++] = x;
			primes[i] = 0;
			if (i == p->n)
				{
					p->c = Chan_new();
					Thread_new((int (*)(void *))sink,
						p, sizeof *p, NULL);
...
int main(int argc, char *argv[]) {
	...
	Thread_init(1, NULL);
	args.c = Chan_new();
	Thread_new((int (*)(void *))source, &args,
		sizeof args, NULL);
	args.n    = argc > 2 ? atoi(argv[2]) : 5;
	args.last = argc > 1 ? atoi(argv[1]) : 1000;
	Thread_new((int (*)(void *))sink,   &args,
		sizeof args, NULL);
